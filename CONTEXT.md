...
- **WebUI client (`src/clients/web/ReelRoulette.WebUI`)**
  ...
  - Library overlay shell (full-screen, filter/tag dialog pattern): **Library** corner control opens overlay inside the fullscreen stage; Close-only header; `GET /api/library/projection` on every open with loading, empty, error, and item-count summary states; playback continues while open. Grid, search, sort, SSE sync, and click-to-play are follow-on work.
