...
- **WebUI client (`src/clients/web/ReelRoulette.WebUI`)**
  ...
  - Library overlay (full-screen, filter/tag dialog pattern): **Library** corner control opens overlay inside the fullscreen stage; header shows **Showing N of M items** beside the title; `GET /api/library/projection` on every open; in-memory browse applies active playback **FilterState** (desktop display-filter parity), free-text search (filename/path), desktop-aligned sort (name, last played, play count, duration, date added) with sort cluster toolbar layout, and a virtualized justified thumbnail grid edge-to-edge in the rounded body (Core-aligned `libraryGridLayout` port, row spacers + 900px overscan, desktop tile chrome, themed overlay scrollbar, `GET /api/thumbnail/{itemId}` when `hasThumbnail`); search/sort persist across close/reopen; browse re-browse resets grid scroll to top; Filter Apply / header preset re-browses without refetch while open; playback continues while open. While the overlay is open, SSE `itemStateChanged` and `playbackRecorded` patch the in-memory projection and refresh the grid (playback-sensitive sort/filter re-browse); `resyncRequired` refetches `/api/library/projection`. Click-to-play remains follow-on work.
