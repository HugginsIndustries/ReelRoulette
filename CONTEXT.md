...
- **WebUI client (`src/clients/web/ReelRoulette.WebUI`)**
  ...
  - Library overlay (full-screen, filter/tag dialog pattern): **Library** corner control opens overlay inside the fullscreen stage; Close-only header; `GET /api/library/projection` on every open; in-memory browse applies active playback **FilterState** (desktop display-filter parity), free-text search (filename/path), and desktop-aligned sort (name, last played, play count, duration, date added) with interim scrollable filename result list; search/sort persist across close/reopen; Filter Apply / header preset re-browses without refetch while open; playback continues while open. Virtual thumbnail grid, SSE sync, and click-to-play are follow-on work.
