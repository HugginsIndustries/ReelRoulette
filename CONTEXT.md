...
- **WebUI client (`src/clients/web/ReelRoulette.WebUI`)**
  ...
  - WebUI includes installable PWA metadata (`manifest.webmanifest`, iOS home-screen meta tags, icons under `public/icons/`) so Add-to-Home-Screen/Install flows run in standalone shell when served from HTTPS; `sync-shared-icon.mjs` (runs before dev/build) uses **`sharp`** to emit **192×192**, **512×512**, and **180×180** PNGs from shared `HI-256.png` / `HI-512.png` so manifest-declared sizes match files.
