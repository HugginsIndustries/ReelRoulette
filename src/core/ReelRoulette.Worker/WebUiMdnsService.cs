...
public sealed class WebUiMdnsService : IHostedService, IDisposable
{
    ...
    public async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Web UI mDNS shutdown encountered an error.");
        }
        finally
        {
            if (lockTaken)
...
#pragma warning restore CS4014
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!snapshot.Enabled || !snapshot.BindOnLan)
            {
                if (_mdns != null)
                {
                    _logger.LogInformation("Web UI mDNS disabled ({Reason}).", reason);
                }
            ...
            _logger.LogInformation("Web UI mDNS enabled ({Reason}) at http://{Host}.local:{Port}/.", reason, hostLabel, port);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to reconcile Web UI mDNS advertisement.");
            StopAdvertisement();
        }
        finally
        {
    ...
    private void StopAdvertisement()
    {
        ...
        {
            _logger.LogDebug(ex, "Web UI mDNS unadvertise failed.");
        }
        finally
        {
