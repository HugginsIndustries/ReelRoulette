...
- **Library list query:** `POST /api/library/query` pages the SQLite catalog in desktop library-panel order: enabled sources, then filename or relative-path search, then filter, then sort. Responses include `totalCount`, `searchBaselineCount`, and thumbnail layout fields for the returned page. Missing files stay in the result. `GET /api/library/projection` remains for current clients.
- **Library catalog cutover:** The server opens `library.db` at startup. A missing database migrates `library.json` and then ignores that JSON file. Catalog mutations update the affected rows in one transaction, including a path rename that only changes casing, and a failed save leaves the previous catalog in place. Playback cache invalidation uses the catalog revision. `GET /api/library/projection` reads that document (integer media type and fingerprint status, `hh:mm:ss` duration) and still adds thumbnail fields at serve time. Duplicate scan reads integer fingerprint status (`0` pending, `1` ready, `2` failed, `3` stale) and still treats a missing status with a SHA-256 fingerprint as ready. Auto-tag scan with `scanFullLibrary: false` and no path list scans enabled sources only. An explicit path list still matches `fullPath`. `scanFullLibrary: true` scans every item. Desktop Library Export and Import, and server `library.json` catalog backups, are unavailable. A refused database stops startup. A new export format is not part of this change.
- **Library catalog session:** Core can open a SQLite catalog session on `library.db` (`user_version` 1, including `loudnessError`). Each operation uses its own connection and updates the affected rows. A committing transaction bumps a catalog revision. The session can rebuild the current library document, with integer media type and fingerprint status and an `hh:mm:ss` duration. A missing database and missing `library.json` create an empty `library.db`. Renaming a tag onto an existing name, or collapsing duplicate names in a catalog sync, keeps the earlier category, unless that category is `Uncategorized` and the other is not. A tag upsert leaves an existing tag's name in place. A blank category leaves that tag's category in place and stores `uncategorized` only for a new tag. A different category updates only the category. Adding a tag to an item inserts a missing catalog tag as `uncategorized` and leaves an existing catalog tag's name and category in place. An inserted item with no fingerprint version stores `1`. Inserted local timestamps are stored as UTC.
- **Library catalog store:** Core can migrate `library.json` into a SQLite `library.db` (WAL, schema `user_version` 1) in the same roaming directory. The database file is synced through a writable handle before it is published. On Windows that publish rename waits until the new name is on disk; on Linux and macOS the directory is synced after publish. `library.json` is renamed only after that succeeds. Publish is skipped when `library.json.migrated` is already present, so `library.json` is not left beside a new database. A partial, unversioned, corrupt, or non-database file is quarantined, including a database whose header opens but whose rows are corrupt. A locked or busy `library.db` fails within about a second and is left in place. An item with no full path is skipped. A refused database names that snapshot only when the file is present. A missing `fingerprintStatus` stays null. A source without an id or root path is skipped. The uncategorized category is always stored as `Uncategorized` with sort order `int.MaxValue`, and it is added when the file does not already have that id. Duplicate tag names collapse case-insensitively to the last one.
...
- **WebUI — library overlay:** Full-screen library browser (browse icon, left of Filter) fetches `GET /api/library/projection` on open, browses in memory with desktop-parity filter/search/sort, renders a virtualized justified thumbnail grid, live-syncs via SSE while open, and plays a tile through `POST /api/play/{itemId}` (overlay closes; no duplicate client `record-playback`). Escape closes; tiles are keyboard-activatable; light/dark and narrow-width toolbar wrap are signed off.
- **Desktop — library grid:** Multi-select, bulk context menu, **Date added** sort (`lastWriteTimeUtc`), and API-only thumbnails from projection metadata plus `GET /api/thumbnail/{itemId}`.
- **Server — direct item play:** `POST /api/play/{itemId}` returns `RandomResponse`, records playback, and emits `playbackRecorded`; OpenAPI documents error `code` values. Shared playable-extension allowlist; library playback projection honors per-source `isEnabled`.
- **Server — projection thumbnail metadata:** `GET /api/library/projection` adds `hasThumbnail`, `thumbnailWidth`, and `thumbnailHeight` from the thumbnail index (not persisted in `library.json`). Shared `LibraryGridLayout` justified-row algorithm with tests.

### Changed

- **Packaging and installation:** Single Velopack path on Windows and Linux. Update feeds are published on B2; ServerApp and the desktop client apply in-app updates when installed via Velopack. Windows release builds install FFmpeg from the latest GitHub-hosted BtbN line into the server package and bundle desktop LibVLC from NuGet publish output; Windows desktop ships one LibVLC tree under `runtimes/win-x64/native/libvlc/`; Linux desktop packages no longer carry unused Windows LibVLC (playback still uses system LibVLC on Linux).
- **Release tooling — version source and staging:** Repo-root `.version` is the canonical release version (v-prefixed semver2). `set-release-version.ps1` reads or writes `.version` and fans out bare semver to csproj, OpenAPI, and test fixtures. WebUI static asset staging and Windows native dependency staging moved to `stage-webui-assets.ps1` and `stage-native-deps.ps1`.
- **Desktop — click-to-play and Force API media:** Grid activation calls `POST /api/play/{itemId}` and starts the returned `RandomResponse` in LibVLC without a duplicate client `record-playback`. Root-relative `/api/media/...` URLs resolve against the core base URL on Linux (including photo display), instead of being treated as `file://` paths.
- **Desktop — API-authoritative filter, random, and presets:** Library panel filters use the server-backed projection; random playback stays on `/api/random` with shuffle/spread keyed by client plus session; active preset labels resolve through `/api/presets/match`. Local `FilterService` / `FilterSetBuilder`, `RandomSelectionEngine`, and preset JSON comparison fallback are gone.
...
- **Legacy installers and packages:** Windows Inno Setup installers and portable ZIP packages; Linux portable tarballs and the legacy AppImage build/`--install` flow (Velopack AppImages register in the application menu on launch).
- **Desktop library list view:** List rendering, the grid/list toggle, and `LibraryGridViewEnabled` settings persistence. A leftover `libraryGridViewEnabled` value in desktop settings is ignored if still present.

### Fixed

- **WebUI light theme (tag editor / filter Tags):** Chip labels and in-chip icons stay white on colored chips; shell icons use dark glyphs in light theme. Pending or selected chip actions still use Huggins Orange on the plus/minus (and filter include/exclude) toggles.
- **Desktop tag editor parity:** Pending add/remove actions in `ItemTagsDialog` tint the selected plus/minus glyph in Huggins Orange.
- **WebUI `/api/random`:** SmartShuffle and SpreadMode are session-scoped so multiple tabs with the same stored client id no longer share one shuffle bag; weighted modes see recent plays because the browser records playback to the library index.
- **Desktop preset auto-detection:** Active preset label follows server-side `/api/presets/match` instead of local serialized `FilterState` equality.
- **Desktop — favorite/blacklist toggles:** Programmatic sync during playback no longer fires toggle change handlers (spurious add/remove status and unintended API writes).
- **Server — LAN hostname discovery on Linux:** The server no longer publishes conflicting network records that could rename the machine; discovery is limited to ordinary home-network addresses (not VPN/tunnel interfaces), with an **Advertise on local network (mDNS)** setting to turn hostname discovery off while still allowing LAN access.
- **Linux — desktop LibVLC and first run:** Loads system LibVLC via `libvlc.so.5` (`libvlc.so` fallback), so a standard VLC install is enough. Missing LibVLC shows a distro-specific install dialog instead of a silent exit; AppImage menu registration installs icons before writing the `.desktop` entry. Install-command copy now lists only verified minimal packages, not the full VLC metapackage on split distros.
- **Updates — empty channel and status contract:** A missing `releases.{channel}.json` (HTTP 404) reports **No releases available on this channel**. OpenAPI `ServerUpdatePhase` includes `noReleases` and `checkFailed`.
...
[Unreleased]: https://github.com/HugginsIndustries/ReelRoulette/compare/v0.12.0...HEAD
...
[0.10.0]: https://github.com/HugginsIndustries/ReelRoulette/compare/v0.9.0...v0.10.0
[0.9.0]: https://github.com/HugginsIndustries/ReelRoulette/releases/tag/v0.9.0
