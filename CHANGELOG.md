...
## Unreleased
- **Add volume normalization system** with three modes:
  - Off: No normalization (direct volume control)
  - Simple: Real-time normalization using LibVLC `normvol` audio filter
  - Library-aware: Per-file loudness scanning with ffmpeg + real-time normalization
...
- Volume normalization uses target loudness of -18 dB with ±6 dB gain adjustment cap
- Library-aware mode automatically adjusts volume per-file based on scanned loudness data
