...
- **Updates — status contract:** OpenAPI `ServerUpdatePhase` (and generated WebUI types) include **`noReleases`** and **`checkFailed`**, matching those runtime phases.
...
- **Desktop — import source names:** Importing a folder now names the source after the folder even when the picker path ends with a slash (common on Linux). The same folder with or without a trailing slash is treated as one source. Existing sources that were stored with a blank name show the folder name in source lists without rewriting `library.json`. Trailing separators are trimmed without collapsing a Windows drive root (`D:\`) into a drive-relative path (`D:`).

### Added

- **Linux — AppImage application menu:** On Linux, Velopack AppImages for the server and desktop client register themselves in the application menu on launch and keep the shortcut pointed at the current AppImage file if you move it; no installer or manual `--install` step.
...
- **Release pipeline — Velopack on B2:** Tag and manual `release.yml` workflow packs ServerApp and DesktopApp with Velopack, publishes dev/stable update feeds to Backblaze B2 (`reelroulette/*` prefixes), and mirrors stable builds to GitHub Releases. Installed hosts call `VelopackApp` hooks at startup and expose in-app check → download → apply (Operator and Desktop Settings).
...
- **WebUI — library overlay click-to-play:** Library tile single-click/Enter/Space calls `POST /api/play/{itemId}` via `coreApi.requestPlayItem`; success closes the overlay and starts returned media through `playCurrent({ skipRecordPlayback: true })` without duplicate client `record-playback`; endpoint errors map to status messages (404/409/415). Pure helpers in `libraryPlayModel.ts`.
...
- **Packaging and installation:** ReelRoulette now ships through a single Velopack installer path on Windows and Linux (replacing Inno Setup, portable ZIP/tarball packages, and legacy AppImage build scripts). Update **feeds** are published on B2; **ServerApp** and the **desktop client** apply in-app updates when installed via Velopack.
- **Release pipeline — Windows native dependencies:** Velopack release builds now install FFmpeg from the latest GitHub-hosted BtbN release build and copy it into the server package, and bundle the desktop video playback library from its NuGet publish output instead of relying on the legacy fetch-and-stage scripts used by Inno/AppImage packaging.
