...
- VLC / LibVLC for desktop video playback when running from source; FFmpeg (with `ffprobe` on `PATH`) on the **server** for library refresh (duration, loudness, thumbnails). Install from your OS on Linux and for local Windows `dotnet run`. **Velopack** Windows server packages bundle FFmpeg/ffprobe; Windows desktop packages bundle LibVLC. Linux AppImages bundle neither—on Debian/Ubuntu/Mint also install **`libvlc-dev`** alongside **`vlc`**. If the desktop AppImage is launched without LibVLC, it shows a copy-paste install command for the detected distro family.
