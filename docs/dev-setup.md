...
- VLC / LibVLC for desktop video playback when running from source; FFmpeg (with `ffprobe` on `PATH`) on the **server** for library refresh (duration, loudness, thumbnails). Install from your OS on Linux and for local Windows `dotnet run`. **Velopack** Windows server packages bundle FFmpeg/ffprobe; Windows desktop packages bundle LibVLC.
...
- `pwsh ./tools/scripts/set-release-version.ps1 -Version v0.12.0-dev.7`
