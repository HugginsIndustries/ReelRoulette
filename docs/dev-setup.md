...
- or run the chained flow:
  - `pwsh ./tools/scripts/full-release.ps1 -Version v0.12.0-dev.2` (optional `-NoDocUpdates`, `-NoUpdateDesktopVersion`, `-NoRegenerateContracts`, `-NoRunVerify` are passed through to `set-release-version.ps1`). Run without `-Version` to skip `set-release-version` and use `.version` in package outputs.
