...
- `GET /api/library/projection` — full catalog document read from the SQLite catalog for current clients. `mediaType` and `fingerprintStatus` are integers and `duration` is `hh:mm:ss`. Each item is enriched at serve time with **`hasThumbnail`**, **`thumbnailWidth`**, and **`thumbnailHeight`** from the server thumbnail index (not stored in the catalog). JPEG bytes come from `GET /api/thumbnail/{itemId}`. This is not the long-term browse API; browse uses `POST /api/library/query`. Desktop and WebUI still fetch this snapshot until they cut over.
