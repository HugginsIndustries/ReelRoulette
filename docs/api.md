...
- `GET /api/library/projection` — returns the library document read from the SQLite catalog; `mediaType` and `fingerprintStatus` are integers and `duration` is `hh:mm:ss`. Each item is enriched at serve time with **`hasThumbnail`**, **`thumbnailWidth`**, and **`thumbnailHeight`** from the server thumbnail index (not stored in the catalog). Clients use these fields for justified-grid layout; JPEG bytes come from `GET /api/thumbnail/{itemId}`. This full-catalog response remains the browse path for current clients.
