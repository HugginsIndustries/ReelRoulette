...
- [ ] **Linux desktop — missing VLC:** On a system without VLC/LibVLC installed, launching the desktop AppImage shows the dependency dialog with a working, copyable install command (including `libvlc-dev` on Debian/Ubuntu/Mint) instead of exiting silently; dismissing the dialog exits cleanly and does not leave a menu entry without its icon.
